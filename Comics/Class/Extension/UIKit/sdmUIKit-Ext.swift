...
enum Device
{
    ...
    static let base: Device = .iPhone5
...
extension CGSize
{
    ...
    func pscale(_ device: Device? = nil) -> CGSize
    {
        let src: Device = device ?? Device.base
        let ramp: CGFloat = 1.0 / CGSize(device: src).width * CGSize(device: Device.current).width
        
        return CGSize(width: self.width * ramp,
                      height: self.height * ramp)
    }
